...
container update is opened, versions compatible with Valkey Bundle trigger its
...
package without receiving AWS or publishing credentials. Empty matrices and
skipped GA package builds fail the summary job.

`valkey-ci-agent` dispatches qualification from its publication workflow and
waits synchronously for the exact request id. The workflow conclusion is the
gate; there is no manifest artifact, nonce receipt, or controller-side parser.
